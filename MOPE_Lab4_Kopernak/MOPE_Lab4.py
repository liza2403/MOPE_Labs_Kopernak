...
def check(X, Y, B, n, m, norm=False):
    ...
    print('Dispersion y:', dispersion_arr)
...
def linear(n, m):
    ...
    print('\nCochren test:\n')
    print(f'Calculated value: Gp = {Gp}'
          f'\nTabular value: Gt = {cohren_cr_table}')
    if Gp < cohren_cr_table:
        print(f'With probability {1-q} the variances are homogeneous.')
    else:
        print("It is necessary to increase the number of experiments")
    ...
    print("The calculated value of the Student's criterion:\n", student_t)
    ...
    print('The coefficients {} are statistically insignificant.'.
    ...
    print(f'\nObtaining the value of the regression level for {m} experiments: ')
    print(y_new)
    ...
    print("\nReconstruction of adequacy for Fisher's criteria:\n")
    print("Estimated value of Fisher's test: Fp =", Fp)
    print("Tabular value of Fisher's test: Ft =", Ft)
    if Fp < Ft:
        print('The mathematical model is adequate to the experimental data')
        return True
    else:
